...
description: "Four deployment workflows for Java apps: standard Git-based pipelines, ZIP source upload, manual docker cp deploys, and parameterized Jenkins jobs with environment selection."
---
...
<Steps>
  ...
  <Step title="What the script does internally">
    ...
    2. **Extracts the ZIP** — unpacks to a temp directory via `unzip`
    ...
    5. **Scans for hardcoded passwords** — greps `src/` for literal password assignments; exits with an error if any are found
    6. **Builds with Maven** — runs `mvn clean install` with the environment flag and a generated build version
    7. **Runs tests** — executes `mvn test`; stops if tests fail
    8. **Deploys the WAR** — copies the built WAR file to the running container via `docker cp`
  </Step>
</Steps>
