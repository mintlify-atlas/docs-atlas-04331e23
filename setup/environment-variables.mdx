...
description: "Complete reference for all environment variables used by Jenkins, SonarQube, Nexus, WildFly, and JBoss containers, plus pipeline and application-level variables."
---
...
| `SONAR_CE_JAVAOPTS` | *(not set by default)* | JVM options for the SonarQube Compute Engine background process. Example: `-Xmx2048m -Xms1024m`. |
| `SONAR_WEB_JAVAOPTS` | *(not set by default)* | JVM options for the SonarQube web server process. Example: `-Xmx2048m -Xms1024m`. |
...
| `JAVA_OPTS` | *(not set by default)* | Optional JVM tuning for the application server. For example: `-Xms2048m -Xmx4096m`. Set this when deploying large Jakarta EE applications that require more heap. |
